...
export class TodosComponent implements OnInit {
  ...
  */
  ...
  selectedTodo: Todo={ sno: 0, title: '', discr:'', startTime: '', endTime: '', date: '',active:true};
  index:number=0;
  editTask(event: Todo): void {
    this.selectedTodo =event;
    this.index = this.todos.indexOf(event);
    console.log("ready to edit, passing it to form", this.index);
    // console.log("selected Todo is", this.selectedTodo);
  ...
  updateTodo(todo: Todo): void {
    ...
    this.updateLocalStorage();
    this.showTodos = true;
    this.selectedTodo = { sno: 0, title: '', discr: '', startTime: '', endTime: '', date: '',active:true};
  }
  /*
